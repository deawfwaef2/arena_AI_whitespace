...
const c=await b.newContext({viewport:{width:W,height:H},screen:{width:W,height:H},isMobile:true,hasTouch:true,deviceScaleFactor:2,userAgent:devices['iPhone 13'].userAgent});
...
await p.waitForTimeout(2500);await p.screenshot({path:'qa/r14-phone-menu.png',timeout:60000});
...
const got=p.getByText('Got it');if(await got.count())await got.first().tap().catch(e=>errs.push('gotit '+e.message.split('\n')[0]));
await p.waitForTimeout(600);const tap=p.locator('[data-action="v9-tap"]').first();
for(let i=0;i<4;i++){await tap.tap({timeout:8000}).catch(e=>errs.push('tap '+e.message.split('\n')[0]));await p.waitForTimeout(250);}
console.log('work progress',await p.evaluate(()=>document.querySelector('.v9-work-bar span')?.textContent));
await p.screenshot({path:'qa/r14-phone-game.png',timeout:60000});
