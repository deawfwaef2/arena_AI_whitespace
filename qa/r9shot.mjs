...
await shot('rest',{cash:5000,energy:0,q:'?adtest=1',after:async p=>{await p.evaluate(()=>{const b=[...document.querySelectorAll('[data-action]')].find(b=>/rest/.test(b.dataset.action)&&b.offsetParent);b?.click();});await p.waitForTimeout(1500);for(let i=0;i<4;i++){await p.evaluate(()=>{const b=[...document.querySelectorAll('#modal-card [data-action]')].find(b=>b.offsetParent&&!/close/.test(b.dataset.action));b?.click();});await p.waitForTimeout(900);}}});
