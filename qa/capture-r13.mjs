...
import {UNLOCK_MILESTONES} from '../src/life-core.js';
const SEEN=r=>{r.life.seenMilestones=UNLOCK_MILESTONES.map(m=>m.id);};
const STEP=66;
...
async function seg(name,{cash=100,city='taipei',secs=5,seed,act}){
 ...
 const dir=(process.env.OUT||'/tmp/frames')+'/'+name;fs.rmSync(dir,{recursive:true,force:true});fs.mkdirSync(dir,{recursive:true});
 ...
 for(let i=0;i<n;i++){
  ...
  let shot;for(let t=0;t<4;t++){try{shot=await cdp.send('Page.captureScreenshot',{format:'jpeg',quality:86,optimizeForSpeed:true});break;}catch(e){console.log(name,'retry shot',i,e.message);await new Promise(r=>setTimeout(r,1500));}}if(!shot)continue;fs.writeFileSync(`${dir}/${String(i).padStart(4,'0')}.jpg`,Buffer.from(shot.data,'base64'));
  if(i%20===0)console.log(name,i,'/',n,((Date.now()-t0)/(i+1)|0)+'ms/f');
 }
 fs.writeFileSync(`${process.env.OUT||'/tmp/frames'}/${name}.json`,JSON.stringify(hints));await c.close();console.log('done',name,n);}
...
await seg('tap',{cash:100,secs:5.4,act:async(i,p,box)=>{
...
await seg('level',{cash:2960,secs:4.3,act:async(i,p,box)=>{
...
await seg('invest',{cash:6000,city:'tokyo',secs:5,seed:r=>{r.page=14;SEEN(r);r.offer=makeOffer(r,{project:true});},act:invest(.02,.92,'win')});
await seg('bigwin',{cash:240000,city:'vegas',secs:4,seed:r=>{r.page=30;r.offer=makeOffer(r,{project:true});},act:invest(.1,1,'win')});
// 4) ALL IN … and lose everything
await seg('allin',{cash:9500000,city:'monaco',secs:6,seed:r=>{r.page=60;SEEN(r);r.offer=makeOffer(r,{project:true});r.offer.maxStake=Math.max(r.offer.maxStake||0,r.cash);},act:invest(.15,1,'lose')});
await b.close();
