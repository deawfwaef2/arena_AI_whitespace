...
const openCrew=async p=>{await p.evaluate(()=>document.querySelector('[data-action="street-companion"]')?.click());await p.waitForTimeout(900);};
await shot('companion',{cash:2000,companions:['guide'],after:openCrew});
await shot('companion-phone',{cash:2000,companions:['guide'],width:390,height:844,after:openCrew});
const toRest=async p=>{await click(p,'life-prompt-rest');await click(p,'life-confirm-start-rest');await p.waitForTimeout(900);
 for(const a of ['life-event-choice','life-event-ack','life-pay'])try{await click(p,a);}catch{}
 await p.waitForTimeout(1200);};
await shot('rest',{cash:2000,energy:0,after:toRest});
await shot('rest-rich',{cash:5000000,energy:0,after:toRest});
await shot('factions',{cash:2000000,after:async p=>{await p.evaluate(()=>document.querySelector('[data-action="menu"]')?.click());await p.waitForTimeout(700);await p.evaluate(()=>{document.querySelector('details')?.setAttribute('open','');});await p.waitForTimeout(300);await p.evaluate(()=>document.querySelector('[data-action="life-factions"]')?.click());await p.waitForTimeout(900);}});
await shot('rest-phone',{cash:2000,energy:0,width:390,height:844,after:toRest});
