...
function makePlot(offer,lang='en'){PLOT_CITY=offer?.city||PLOT_CITY;
 if(offer.type==='v12-ad'||offer.type==='v13-stroll'||offer.type==='v13-promo'||offer.type==='v14-sponsor')return sponsorPlot(offer,lang);
...
export class World{
 constructor(container,{onError=()=>{}}={}){
  ...
  try{
   this.renderer=new T.WebGLRenderer({antialias:true,alpha:true,powerPreference:'high-performance'});this.renderer.setPixelRatio(window.__vdesk?Math.min(devicePixelRatio,1.2):Math.min(devicePixelRatio*(window.__vscale||1),(window.__vscale||1)>1?1.95:1.5));this.renderer.shadowMap.enabled=true;this.renderer.shadowMap.type=T.PCFShadowMap;this.renderer.outputColorSpace=T.SRGBColorSpace;this.renderer.toneMapping=T.ACESFilmicToneMapping;this.renderer.toneMappingExposure=1.08;container.appendChild(this.renderer.domElement);
 ...
 quality(low){if(!this.renderer)return;this.renderer.setPixelRatio(low?1:window.__vdesk?Math.min(devicePixelRatio,1.2):Math.min(devicePixelRatio*(window.__vscale||1),(window.__vscale||1)>1?1.95:1.5));this.renderer.shadowMap.enabled=!low;this.resize();}
