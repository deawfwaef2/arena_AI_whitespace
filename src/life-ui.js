...
import {MAX_ENERGY,CITY_DATA,ITEMS,CLASSES,MECHANISMS,getCity,owns,classIndex,initLife,markLife,eligible,stakeBounds,buyUtility,beginRest,payRest,tickRest,restActivity,rewardRest,adSkipRest,finishRest,routes,startTravel,arrive,dailyRate,offlineIncome,globalRank} from './life-core.js';
...
export class LifeUI{
 ...
 renderRest(){
  ...
  ${this.c.platform.canReward()&&!r.adSkip?`<button class="rest-adskip" data-action="life-adskip" ${this.adBusy?'disabled':''}><img src="${window.UPSHIFT_ART?.['ic-bolt']||''}" alt=""><span><b>${'看一段广告 · 立即结束休息'}</b><small>${'完整清空剩余计时，体力回满 · 每次休息一次'}</small></span><em>AD</em></button>`:''}
 ...
 async handle(a,v){
  ...
  try{
   switch(a.slice(5)){
    ...
    case 'arrive':{const city=arrive(s);if(Date.now()-(window.__lastMidgame||0)>180000&&this.c.platform?.midgame){window.__lastMidgame=Date.now();this.c.platform.midgame();}s.page++;s.offer=makeOffer(s);s.lastResult=null;this.c.save();this.c.world.setOffer(s.offer);this.c.refresh();this.c.renderDock();this.c.toast(`欢迎来到${city.name}。${city.tag}`);break;}
