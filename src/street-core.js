...
export function hire(s,id,currentWorth){const t=TALENTS.find(t=>t.id===id);if(s.ended||s.life?.rest||s.life?.travel)throw Error('请返回街头再签约。');if(!t||t.city!=='any'&&t.city!==s.life.city)throw Error('这位人才只在当地市场签约。');if(hasCrew(s,id))throw Error('这位伙伴已经随行。');if(crew(s).length>=3)throw Error('最多同时雇佣三位随从。');if(currentWorth<t.at||s.cash<=t.fee)throw Error('未达到身家门槛，或签约后没有剩余现金。');s.cash-=t.fee;s.life.companions=[...crew(s).map(t=>t.id),id];return t;}
...
export function passer(s,i){const n=(s.page*7+i*13+regions.indexOf(CITY_STREET[s.life.city]?.manager))>>>0;const rank=ranks[(i+s.page)%ranks.length],f=factions[(i+s.page)%factions.length];return {id:`${s.life.city}:${s.page}:${i}`,name:names[(i+s.page)%names.length],origin:i%3===0?regions[(i+s.page)%6]:({taipei:'台北',tokyo:'东京',singapore:'新加坡',newyork:'纽约',monaco:'摩纳哥',vegas:'拉斯维加斯'})[s.life.city],cash:rank[1]+n*113,rank:rank[0],faction:f[0],factionName:f[1],color:[0x7c9e89,0xb39a7b,0x929eb8,0xa6818e,0x789ca3,0xb6aa77][i%6]};}
