...
export class V12{
 ...
 dockShop(o){const K=X.SHOP_KINDS[o.kind]||X.SHOP_KINDS.kiosk,s=this.s,big=K.size==='b';
  const tiles=o.goods.map((it,i)=>{const g=X.findGood(s,it.id);if(!g)return '';const cat=X.SHOP_CATS[g.cat];const f=X.famOf(g);const price=f&&!it.sold?X.goodPrice(s,g):it.price;const dis=it.sold||s.cash<=price;const lk=X.goodLook(s,g);
   const xm=f==='risk'?3:2;const badge=f?`<span class="v12-g-x2" title="${this.T(`同类商品每买一次，价格 ×${xm}、效果减弱`,`Each purchase in this family multiplies the price by ${xm} and gives less`)}">${lk.n?this.T(`第 ${lk.n+1} 次 · `,`#${lk.n+1} · `):''}${this.T(`再买 ×${xm}`,`next ×${xm}`)}</span>`:g.lv?`<span class="v12-g-x2" title="${this.T('每买一件 LV 收藏，下一件价格翻倍','Every LV purchase doubles the next LV price')}">${this.T('LV · 再买 ×2','LV · next ×2')}</span>`:g.item?`<span class="v12-g-x2 unl">${this.T('新机制','NEW')}</span>`:'';
   return `<button class="v12-good ${it.sold?'sold':''} ${g.item?'is-unlock':''}" style="--cat:${cat.color}" data-action="v12-buy" data-value="${i}" ${dis?'disabled':''}><span class="v12-g-cat">${safe(this.P([cat.zh,cat.en]))}</span>${badge}${img(lk.icon,'v12-g-img')}<b class="v12-g-name">${safe(this.P([lk.zh,lk.en]))}</b><small class="v12-g-eff">${safe(this.P(lk.d))}</small><span class="v12-g-price">${it.sold?this.T('已售出','SOLD'):this.money(price)}</span></button>`;}).join('');
 ...
 dockCity(o){const S=X.CITY_STYLE[o.city],d=X.CITY_DEALS[o.city]?.find(x=>x.id===o.deal);if(!S||!d)return `<p>…</p>${`<button class="v9-next" data-action="next">${img('compass','v9-ic sm')}<span>${this.T('下一站','Next stop')}</span></button>`}`;const s=this.s;
  ...
  const lowE=s.life.energy<3;const maxOk=Math.max(o.min,Math.min(o.max,s.cash-1));o.stake=clamp(o.stake||o.min,o.min,maxOk);const ST=this.stakeTxt(o);
 ...
 stakeTxt(o){const S=X.CITY_STYLE[o.city]||{};const st=o.stake;let up=o.up;if(S.lever)up=+(o.up+.6*((st-o.min)/Math.max(1,o.max-o.min))).toFixed(2);return {v:this.money(st),w:'+'+this.money(Math.floor(st*up+1e-6)-st)+' (×'+up+')',l:'−'+this.money(S.half?Math.floor(st/2):st)};}
 paintStake(){const o=this.s.offer;if(o.type!=='v12-city'||o.settled)return;const S=X.CITY_STYLE[o.city]||{};const st=o.stake;let up=o.up;if(S.lever)up=+(o.up+.6*((st-o.min)/Math.max(1,o.max-o.min))).toFixed(2);
  const set=(id,t)=>{const e=$(id);if(e)e.textContent=t;};set('v12-stake-v',this.money(st));set('v12-stake-w','+'+this.money(Math.floor(st*up+1e-6)-st)+' (×'+up+')');set('v12-stake-l','−'+this.money(S.half?Math.floor(st/2):st));
 ...
 dockHospital(o){const s=this.s,v=C.st(s),H=X.HOSPITAL_TEXT[this.zh?'zh':'en'],uses=v.hospUses||0,mult=X.hospMult(s);const opts=C.PLACE_OPTS.hospital.filter(x=>o.opts.includes(x.id));
  ...
  const tiles=opts.map(d=>{const q=C.quoteOpt(s,d);const full=d.heal&&s.estate.health>=s.estate.maxHealth;const dis=(q.cost&&q.cost>=s.cash)||full;const pct=d.heal?q.healP:d.riskP?d.riskP:d.risk?100-d.risk:null;const det=X.HOSP_DETAIL[d.id];
  ...
  return `<div class="v12-hosp"><div class="v12-hosp-l"><div class="v12-hosp-top">${img('hospital','v12-hosp-img')}<div class="v12-hosp-txt"><span class="v9-tag">${img('compass','v9-ic sm')}${this.T('特殊地点 · 医院','Special place · Hospital')}</span><h1>${this.T('圣心综合医院','Sacred Heart General')}</h1><div class="v9-hearts">${hearts}</div></div><div class="v12-visit"><small>${this.T('就诊次数','Visits')}</small><b>${uses}</b><small>${this.T('价格','Prices')} ×${mult}</small></div></div>
   <div class="v12-hosp-story"><p>${safe(H.smell)}</p><p>${safe(H.desk)}</p><p class="doc">${safe(H.doctor[Math.min(2,uses)])}</p><p class="warn">${img('seal','v9-ic sm')}${safe(H.wall)}</p></div></div>
   <div class="v12-hosp-r"><div class="v12-hosp-opts">${tiles}</div><button class="v9-leave" data-action="next">${img('compass','v9-ic')}<b>${this.T('离开医院','Leave the hospital')}</b></button></div></div>`;}

 dockDen(o){const s=this.s,T=X.PLACE_TEXT[o.place],H=T[this.zh?'zh':'en'],Z=C.ZONES[o.place];const all=C.PLACE_OPTS[o.place],opts=all.filter(x=>o.opts.includes(x.id));const voice=H.voice[(String(o.id).length+s.page)%H.voice.length];
  const tiles=opts.map(d=>{const q=C.quoteOpt(s,d);const det=T.detail[d.id];const dis=(q.stake&&q.stake>=s.cash)||(q.cost&&q.cost>=s.cash);const nums=q.stake?`<em class="v12-ho-p">${q.p}%</em><b>−${this.money(q.stake)}</b><small class="w">+${this.money(q.win)}</small>`:q.cost?`<b>−${this.money(q.cost)}</b>`:q.gain?`<b class="w">+${this.money(q.gain)}</b><small>${this.T('还','repay')} ${this.money(Math.floor(q.gain*1.3))}</small>`:'';
   return `<button class="v12-hosp-opt v12-den-opt" data-action="v9-place-go" data-value="${all.indexOf(d)}" ${dis?'disabled':''}>${img(d.icon,'v12-ho-ic')}<span class="v12-ho-t"><b>${safe(this.P([d.zh,d.en]))}</b><small>${safe(det?this.P(det):this.P(d.desc))}</small></span><span class="v12-ho-s">${nums}</span></button>`;}).join('');
  return `<div class="v12-hosp v12-den d-${o.place}"><div class="v12-hosp-l"><div class="v12-hosp-top">${img(Z.icon,'v12-hosp-img')}<div class="v12-hosp-txt"><span class="v9-tag">${img('compass','v9-ic sm')}${this.T('特殊地点','Special place')}</span><h1>${safe(this.P(T.name))}</h1></div></div><div class="v12-hosp-story"><p>${safe(H.smell)}</p><p>${safe(H.desk)}</p><p class="doc">${safe(voice)}</p><p class="warn">${img('seal','v9-ic sm')}${safe(H.wall)}</p></div></div><div class="v12-hosp-r"><div class="v12-hosp-opts">${tiles}</div><button class="v9-leave" data-action="next">${img('compass','v9-ic')}<b>${this.T('转身离开','Walk away')}</b></button></div></div>`;}
