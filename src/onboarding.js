...
export class Onboarding{
 constructor(ctx){this.c=ctx;this.preview={screen:'street',wealth:'poor',device:ctx.meta().device||'desktop'};this.active=false;this.step=0;this.mode='start';this.element=document.createElement('section');this.element.id='start-screen';this.element.className='start-screen';this.element.setAttribute('role','dialog');this.element.setAttribute('aria-modal','true');this.element.setAttribute('aria-labelledby','start-title');document.getElementById('app').append(this.element);window.addEventListener('upshift-art',()=>this.artArrived());
 ...
 artArrived(){const h=this.element.querySelector('.r12-house'),a=window.UPSHIFT_ART||{};if(h&&a.monaco)h.style.backgroundImage=`url('${a.monaco}')`;}
