...
export function medicalOptions(s){const n=worth(s),u=s.estate.medicalUses;return [{id:'care',name:'私人康复疗程',min:1e5,cost:Math.max(250000,Math.floor(n*.018))*2**Math.min(20,u),desc:'恢复 1 格健康，不改变下一次抽签概率。'}, {id:'longevity',name:'延寿研究合约',min:1e7,cost:Math.max(100000000,Math.floor(n*.09))*2**Math.min(20,u),desc:'衰退概率降低 6 个百分点；每次休息仍继续增长。'}, {id:'renewal',name:'细胞更新计划',min:1e10,cost:Math.max(100000000000,Math.floor(n*.25))*2**Math.min(20,u),desc:'健康恢复至上限，衰退概率降低 12 个百分点。'}].map(o=>({...o,cost:cents(o.cost)}));}
export function buyMedical(s,id){const e=s.estate,o=medicalOptions(s).find(x=>x.id===id);if(!o||worth(s)<o.min*100||s.ended)throw Error('尚未达到医疗项目门槛。');if(pendingEvent(s))throw Error('请先完成当前事件，不能改变已经抽出的结果。');if(id==='care'&&e.health>=e.maxHealth)throw Error('健康已经满格。');if(s.cash<=o.cost)throw Error('现金不足。');s.cash-=o.cost;e.medicalUses++;if(id==='care')e.health=Math.min(e.maxHealth,e.health+1);if(id==='longevity')e.riskReduction+=6;if(id==='renewal'){e.health=e.maxHealth;e.riskReduction+=12;}reconcile(s);return o;}
