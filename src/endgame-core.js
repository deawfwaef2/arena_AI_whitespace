...
export const fatigueExtra=en=>Math.round(100*Math.pow(Math.max(0,Math.min(100,100-en))/100,2));
export function healthRisk(s,next=false){return Math.round(100*clamp((s.estate?.age||0)+(next?1:0)-(s.estate?.riskReduction||0)-(hasCrew(s,'medic')?3:0),0,100))/100;}
...
export function prepareRest(s,r,rng=Math.random){const e=ensureEstate(s);e.age++;const bill=billQuote(s,{restNumber:s.life.restCount});Object.assign(r,{maintenance:bill.maintenance,tax:bill.tax,extra:bill.guards+bill.management+(bill.timeFee||0)-bill.credit,bill,paid:false});
 const en=Number(s.life.v9fatigue),fat=Number.isFinite(en)&&en<100?fatigueExtra(en):0;addEvent(s,'health',{risk:healthRisk(s),roll:rng()*100,fat,roll2:rng()*100});
...
export function medicalOptions(s){const n=worth(s),u=s.estate.medicalUses;return [{id:'care',name:'私人康复疗程',min:1e5,cost:Math.max(750000,Math.floor(n*.05))*2**Math.min(20,u),p:55,desc:'55% 概率恢复 1 格健康；失败也照样收费。'}, {id:'longevity',name:'延寿研究合约',min:1e7,cost:Math.max(300000000,Math.floor(n*.2))*2**Math.min(20,u),p:45,desc:'45% 概率：衰退概率降低 4 个百分点；失败也照样收费。'}, {id:'renewal',name:'细胞更新计划',min:1e10,cost:Math.max(300000000000,Math.floor(n*.4))*2**Math.min(20,u),p:60,desc:'60% 概率：健康恢复至上限，衰退概率降低 8 个百分点；失败也照样收费。'}].map(o=>({...o,cost:cents(o.cost)}));}
export function buyMedical(s,id,rng=Math.random){const e=s.estate,o=medicalOptions(s).find(x=>x.id===id);if(!o||worth(s)<o.min*100||s.ended)throw Error('尚未达到医疗项目门槛。');if(pendingEvent(s))throw Error('请先完成当前事件，不能改变已经抽出的结果。');if(id==='care'&&e.health>=e.maxHealth)throw Error('健康已经满格。');if(s.cash<=o.cost)throw Error('现金不足。');s.cash-=o.cost;e.medicalUses++;const ok=rng()*100<(o.p||100);if(ok){if(id==='care')e.health=Math.min(e.maxHealth,e.health+1);if(id==='longevity')e.riskReduction+=4;if(id==='renewal'){e.health=e.maxHealth;e.riskReduction+=8;}}reconcile(s);return {...o,ok};}
...
export function streetEvent(s,rng=Math.random){if(pendingEvent(s)||s.ended||s.life.district||s.activeChallenge)return null;const tier=lateTier(s);if(tier<2)return null;let ev;
 ...
 if(!ev&&rng()<Math.min(.33,.08+tier*.02)){let list=FACTIONS.filter(f=>tier>=f.at);if(list.length>1)list=list.filter(f=>f.id!==s.estate.lastFaction);const f=list[Math.floor(rng()*list.length)],relation=s.estate.relations[f.id];s.estate.lastFaction=f.id;s.estate.factionV=((s.estate.factionV||0)+1+Math.floor(rng()*2))%3;ev=addEvent(s,relation>=40&&rng()<.3?'windfall':'faction',{faction:f.id,hostile:relation<=-35,v:s.estate.factionV});}
...
export function eventView(s,ev){const e=s.estate;
 ...
 if(ev.kind==='health')return {tag:'HEALTH DRAW / 第 '+e.age+' 次休息',title:'今晚，你的身体会怎么回答？',body:`当前健康 ${e.health}/${e.maxHealth}。本次衰退概率 ${ev.risk.toFixed(0)}%，抽中仅损失 1 格，归零则人生结束。${ev.fat?`【过劳】你带着疲惫硬撑到休息：额外 ${ev.fat}% 概率再掉 1 格。`:''}结果在进入休息时已锁定，刷新不会重抽。`,choices:[{id:'reveal',label:'揭晓体检结果',detail:'本次没有现金费用。',cost:0}]};
 ...
 const names={people:[['社区希望你伸手','为街区公共空间提供资金'],['老街坊办百家宴','为街坊宴席和孩子们的课后班出钱'],['菜市场屋顶漏雨','资助市场修缮，摊贩们会记住你']],tech:[['科技联盟发来合作书','支持开放研发与独立技术'],['黑客马拉松缺赞助','为 48 小时编程赛提供奖金池'],['开源实验室求助','为公共实验室的服务器买单']],industry:[['工业联合需要周转','参与供应链保障计划'],['工厂夜班缺设备','资助一条老生产线的安全改造'],['码头工会找上门','为工人的保险基金注资']],state:[['地区议政署向你筹资','认领地区公共建设份额'],['市政厅的剪彩邀请','为新公园冠名出资'],['选举季的捐款信','为一位候选人的竞选活动出钱']],underworld:[['一封没有落款的账单','为所谓的“平静”支付代价'],['停车场里的黑色轿车','车窗摇下：“保护费，这个月涨了。”'],['赌档老板请你喝茶','茶很好，价钱写在杯底']],capital:[['资本公会邀请你坐下','支持私人资本合作网络'],['私人俱乐部的会费单','一张烫金的入会邀请'],['家族办公室的联合投资','跟投一轮“只对朋友开放”的基金']]};const [title,desc]=names[ev.faction][(ev.v||0)%3];const fraction=ev.faction==='state'?.055:ev.faction==='underworld'?.04:.025;
...
export function resolveEvent(s,id,choice){const ev=pendingEvent(s);if(!ev||ev.id!==id||ev.resolved||s.ended)throw Error('此事件已经改变或结算。');const view=eventView(s,ev),opt=view.choices.find(c=>c.id===choice);if(!opt)throw Error('无效选项。');if(opt.cost>=s.cash&&opt.cost>0)throw Error('现金不足，需保留至少 $0.01。');const before=s.cash;let body='',title='你的选择，已经生效。';
 if(ev.kind==='health'){const tot=Math.min(100,ev.risk+(ev.fat||0)),hit=ev.roll<tot,fhit=hit&&ev.roll>=ev.risk;if(hit)s.estate.health=Math.max(0,s.estate.health-1);body=`抽签数 ${ev.roll.toFixed(2)} / 100，衰退阈值 ${tot.toFixed(0)}${ev.fat?`（其中过劳 +${ev.fat}）`:''}。${hit?(fhit?'过劳让你倒下：健康 −1':'健康 −1'):'平安度过，健康未减少'}，现在 ${s.estate.health}/${s.estate.maxHealth} 格。`;title=hit?'身体收走了一格。':'今晚，平安无事。';if(s.estate.health===0)endLife(s,'健康耗尽，生命走到终点');}
