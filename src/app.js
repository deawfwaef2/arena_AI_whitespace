...
import {runIntro,runsDone as runsDoneOf} from './intro.js';
import {installI18n,syncLanguage} from './i18n.js';
import {R9} from './r9.js';
import {R13} from './r13.js';
...
import {classIndex,initLife,stakeBounds,owns,tickRest,UNLOCK_MILESTONES,nextUnlock,checkNewUnlocks,getCity} from './life-core.js';
...
import {Sound,audioGate} from './sound.js';
...
let lifeUI,onboarding,designUI,streetUI,prosperityUI,v7,v9,v12,r9,r13;let started=false;
$('app').innerHTML=shell;
const platform=new Platform(CONFIG);await platform.init();
const defaults={lang:'en',theme:'minimalist',layout:'journal',device:'desktop',scale:1,sound:true,music:true,ambience:true,volume:.28,sfxVolume:1,ambVolume:.5,musicMode:'auto',motion:!matchMedia('(prefers-reduced-motion: reduce)').matches,low:false,name:'',records:[],bots:[],runCount:1};
function normalizeMeta(input={}){const m={...defaults,...input};m.lang=m.lang==='zh'&&(m.introLang||m.langSet)?'zh':'en';m.name=String(m.name||'').slice(0,20);m.theme=['minimalist','imperial','cyber','swiss'].includes(m.theme)?m.theme:'minimalist';m.scale=Math.max(.75,Math.min(1.25,Number(m.scale)||1));m.records=Array.isArray(m.records)?m.records.filter(x=>x&&Number.isFinite(x.peak)).slice(0,25):[];m.bots=Array.isArray(m.bots)?m.bots.filter(x=>x&&x.simulated&&Number.isFinite(x.peak)).slice(0,100):[];m.volume=Math.max(0,Math.min(1,Number(m.volume)||0));for(const [k,d] of [['sfxVolume',1],['ambVolume',.5]]){const v=Number(m[k]);m[k]=Number.isFinite(v)?Math.max(0,Math.min(1,v)):d;}m.musicMode=['auto','city','rush'].includes(m.musicMode)?m.musicMode:'auto';initLegacy(m);return m;}
...
const effects=new Effects($('effects'));const music=new Music();const sound=new Sound();window.__r15sound=sound;
...
window.__r14world=world;window.__r14music=music;window.__platform=platform;
...
let wiping=false;function save(){if(wiping)return;if(run.life){run.life.lastSeen=Date.now();tickRest(run);}record();platform.save(SAVE_KEY,JSON.stringify({version:3,run,meta}));lastSave=performance.now();}
...
function musicMode(){if(!started){music.setMode('menu');return;}const id=meta.musicMode==='auto'?(run.activeChallenge||run.offer.type==='special'?'rush':'city'):meta.musicMode;music.setMode(lifeUI?.musicId(id)||id);}
function preferences(){$('app').dataset.largeText=String(!!meta.largeText);effects.sound=meta.sound;effects.motion=meta.motion;effects.low=meta.low;effects.muted=platform.muted||document.hidden||adPlaying;world.motion=meta.motion;world.quality(meta.low);music.enabled=meta.music;music.setVolume(meta.volume);music.setMuted(platform.muted||adPlaying);music.setHidden(document.hidden);musicMode();sound.setSfx(meta.sound);sound.setVolumes(meta.sfxVolume??1,meta.ambVolume??.5);effects.vol=meta.sfxVolume??1;sound.setAmbience(meta.ambience!==false);sound.setMuted(platform.muted||adPlaying);document.body.dataset.motion=meta.motion?'on':'off';document.body.dataset.theme=meta.theme||'minimalist';document.documentElement.dataset.theme=meta.theme||'minimalist';document.documentElement.style.setProperty('--ui-scale',meta.scale||1);}
...
function pause(){lastClock=performance.now();try{sound.setStreet(started&&!run.ended&&!adPlaying&&!run.life?.rest,modalType||visit?.45:1);if(started)sound.setTier(classIndex(run));}catch{}world.paused=!started||!!modalType||adPlaying||run.ended||!!run.life?.rest||!!run.life?.travel;world.sceneSuspended=!!modalType||adPlaying||run.ended||!started;music.setDucked(!!modalType||!!visit||run.ended);platform.play(started&&!modalType&&!visit&&!run.ended&&!adPlaying&&!run.life?.rest&&!run.life?.travel);renderChallengeHud();}
...
function pulseClock(now){clock();lifeUI?.tick();streetUI?.tick(now);prosperityUI?.tick(now);v7?.tick(now);v12?.tick(now);v9?.tick(now);r9?.tick(now);r13?.tick(now);renderChallengeHud();if(run.activeChallenge&&now-lastSave>1000)save();if(started&&!busy&&!modalType&&!adPlaying){if(run.ended&&!lifeUI?.hasPendingDeath())showGameOver();else if(pendingChallenge.length)showChallengeResult(pendingChallenge.shift());}requestAnimationFrame(pulseClock);}
...
function langRow(){const st=on=>on?'background:linear-gradient(180deg,#ffe9a6,#d9a441)!important;color:#2b1c02!important;border:2px solid #fff3c4!important;box-shadow:0 6px 18px rgba(0,0,0,.35)!important':'background:rgba(255,255,255,.08)!important;color:#fff!important;border:2px solid rgba(255,255,255,.28)!important;box-shadow:none!important';return `<div class="r14-lang" data-notr><span>Language · 语言</span><div><button data-action="set-lang" data-value="en" class="${meta.lang==='en'?'on':''}" style="${st(meta.lang==='en')}"><b style="color:inherit!important;font-size:22px">English</b><small style="color:inherit!important;opacity:.8">English</small></button><button data-action="set-lang" data-value="zh" class="${meta.lang==='zh'?'on':''}" style="${st(meta.lang==='zh')}"><b style="color:inherit!important;font-size:22px">中文</b><small style="color:inherit!important;opacity:.8">简体中文</small></button></div></div>`;}
function showMenu(){
 ...
 openModal('menu','给旅程留一点空白。','功能集中在这里，街头只留下重要选择。',`${langRow()}<div class="menu-grid">
...
// R16b: volume sliders (music / sound effects / street ambience), shown in Settings and in the Music panel.
function volumeRows(){const row=(id,label,v,vid)=>`<label for="${id}" class="small-rule">${label} <strong id="${vid}">${Math.round(v*100)}%</strong></label><input class="volume-slider" id="${id}" type="range" min="0" max="100" value="${Math.round(v*100)}">`;
 return row('music-volume',L('Music volume','音乐音量'),meta.volume,'volume-value')+row('sfx-volume',L('Sound effects volume','音效音量'),meta.sfxVolume??1,'sfx-volume-value')+row('amb-volume',L('Street ambience volume','环境音音量'),meta.ambVolume??.5,'amb-volume-value');}
function showSettings(){
 ...
 ${langRow()}
 <div class="r16-vol">${volumeRows()}</div>
 ...
 <section class="backup-section r13-wipe"><h3>${L('Delete all save data','清空全部存档')}</h3><p>${L('Erases everything on this device/account: current run, cash, legacy, records, runs completed and all settings. The game restarts like the very first time. This cannot be undone.','清除本设备/账号上的一切：当前进度、金钱、传承、记录、已完成局数以及所有设置。游戏将像第一次打开一样重新开始。此操作无法撤销。')}</p><button class="small-button danger" data-action="r13-wipe">${L('Delete all data…','清空全部数据…')}</button></section>
...
function showLeaderboard(tab=boardTab){
 boardTab=tab;record();const tabs=`<div class="tabs"><button class="${tab==='local'?'active':''}" data-action="board-tab" data-value="local">${L('LOCAL RUNS','本机战绩')}</button>${platform.target==='web'||platform.target==='crazygames'?`<button class="${tab==='platform'?'active':''}" data-action="board-tab" data-value="platform">CrazyGames</button>`:''}</div>`;
...
async function restart(){
 ...
 const counted=run.ended||run.page>=3;meta.runsDone=(meta.runsDone??Math.max(0,(meta.runCount||1)-1))+(counted?1:0);
 if(!run.ended)endLife(run,cause);
 save();forceClose();busy=true;$('game').inert=true;
 await playObituary(!meta.motion, cause);
 $('game').inert=false;
 if(run.runNumber%3===0||platform.ads)await platform.midgame();
...
document.addEventListener('click',async e=>{
 ...
 music.unlock();effects.unlock();if(meta.motion){button.animate([{scale:1},{scale:.95},{scale:1}],{duration:220,easing:'ease-out'});const ripple=document.createElement('i');ripple.className='button-ripple';button.append(ripple);setTimeout(()=>ripple.remove(),600);}
 if(v12?.handle(a,v))return;if(v9?.handle(a,v))return;if(v7?.handle(a,v))return;
 ...
 case 'r13-wipe':confirmDialog(L('Delete ALL save data?','确定清空全部存档？'),L('Your run, money, legacy, records, completed-run count and settings will be erased for good. The game will reload as if it were your first visit.','进度、金钱、传承、记录、已完成局数与设置都将被永久删除，游戏会像第一次打开一样重新载入。'),()=>{wiping=true;platform.remove(SAVE_KEY);try{for(const k of Object.keys(localStorage))if(/^(upshift|last100|v8|r9|r1\d)/i.test(k))localStorage.removeItem(k);sessionStorage.clear();}catch{}setTimeout(()=>location.reload(),150);});break;
 ...
 case 'shop-select':selectedOutfit=Number(v);renderDock();break;
 ...
 case 'sound-toggle':meta.sound=!meta.sound;settingsChanged();break;case 'ambience-toggle':meta.ambience=meta.ambience===false;settingsChanged();break;
 ...
 case 'set-lang':{const back=modalType;if(meta.lang!==v)setLanguage();if(back==='menu')showMenu();else showSettings();break;}
...
document.addEventListener('input',e=>{
 ...
 if(e.target.id==='sfx-volume'||e.target.id==='amb-volume'){const v=Number(e.target.value)/100;if(e.target.id==='sfx-volume'){meta.sfxVolume=v;effects.vol=v;}else meta.ambVolume=v;sound.setVolumes(meta.sfxVolume??1,meta.ambVolume??.5);$(e.target.id+'-value').textContent=e.target.value+'%';}
...
document.addEventListener('change',async e=>{
 ...
 if(e.target.id==='music-volume'||e.target.id==='sfx-volume'||e.target.id==='amb-volume'||e.target.id==='ui-scale-slider')save();if(e.target.id==='sfx-volume')sound.ui('tap');
...
/* R15: every user gesture unlocks audio (iOS only honours some event types, so listen to all of them). */
{const gesture=()=>{music.unlock();sound.unlock();effects.unlock();};for(const ev of ['pointerdown','touchend','click','keydown'])document.addEventListener(ev,gesture,{capture:true,passive:true});}
/* R15: recorded click feedback for EVERY button, including the main menu (menu handlers return before the old tone). */
document.addEventListener('click',e=>{const b=e.target.closest?.('button,[data-action],[role=button],.menu-tile,a[href]');if(!b||b.disabled)return;const a=b.dataset?.action||'';if(a==='onboard-play'&&platform.ads)platform.preroll();
 const kind=/(^|-)(close|back|cancel|skip|no)($|-)/.test(a)||b.classList.contains('modal-close')?'back':/(onboard-play|^start|play$|confirm|invest|settle|buy|accept)/.test(a)?'confirm':/toggle|set-lang|switch/.test(a)?'toggle':/^(menu|settings|music|help|history|leaderboard|collection|wardrobe)$/.test(a)?'open':'tap';sound.ui(kind);},true);
document.addEventListener('pointerover',e=>{if(e.pointerType!=='mouse')return;const b=e.target.closest?.('button,.menu-tile,[data-action]');if(!b||b.disabled||b.contains(e.relatedTarget))return;if(!started||modalType||b.closest('#start-screen,.onboarding,#modal'))sound.ui('hover');},true);
...
document.addEventListener('visibilitychange',()=>{lastClock=performance.now();effects.muted=platform.muted||document.hidden||adPlaying;music.setHidden(document.hidden);sound.setHidden(document.hidden);world.last=performance.now();if(document.hidden)save();});
window.addEventListener('pagehide',()=>{clock();save();});
platform.onSettings=settings=>{effects.muted=!!settings.muteAudio||document.hidden||adPlaying;music.setMuted(!!settings.muteAudio||adPlaying);sound.setMuted(!!settings.muteAudio||adPlaying);renderHud();renderMusicStatus();};
platform.onAdState=value=>{adPlaying=value;effects.muted=value||platform.muted;music.setMuted(value||platform.muted);sound.setMuted(value||platform.muted);lastClock=performance.now();world.paused=value||!!modalType||!started||!!run.life?.rest;world.sceneSuspended=value||!!modalType||!started;};
globalThis.__upshiftArtRefresh=()=>{const vf=document.getElementById('v9-frame');if(vf)vf.dataset.key='';refreshStyle();renderHud();if(started)renderDock();if(onboarding?.active&&runsDoneOf(meta)>=7)onboarding.render();};
...
designUI=installRedesign({run:()=>run,meta:()=>meta,world});streetUI=new StreetUI({cash:(c,el)=>cashFlow.play(c,el),run:()=>run,started:()=>started,modal:()=>modalType,busy:()=>busy,world,save,fit:fitDock,refresh:()=>{refreshStyle();renderHud();renderDock();},open:openModal,close:forceClose,confirm:confirmDialog,toast});prosperityUI=new ProsperityUI({run:()=>run,started:()=>started,motion:()=>meta.motion,blocked:()=>busy||!!modalType||adPlaying||!!run.life.rest||!!run.life.travel,unlock:checkMilestoneCelebration,feedback:up=>{effects.tone(up?'rare':'loss',4);if(up){const r=$('game').getBoundingClientRect();effects.burst(r.width*.45,170,6,'#dbbb66');world.celebrate();}}});v9=new V9({run:()=>run,meta:()=>meta,started:()=>started,modal:()=>modalType,busy:()=>busy,motion:()=>meta.motion,save,toast,open:openModal,close:forceClose,refresh:()=>{refreshStyle();renderHud();},renderDock,fit:fitDock,cash:(c,el)=>cashFlow.play(c,el),effects,world,money:c=>money(c,true),next:()=>{busy=false;nextOffer();},death:()=>{endLife(run,'健康耗尽');save();showGameOver();}});v12=new V12({...{run:()=>run,meta:()=>meta,started:()=>started,modal:()=>modalType,busy:()=>busy,motion:()=>meta.motion,save,toast,open:openModal,close:forceClose,refresh:()=>{refreshStyle();renderHud();},renderDock,fit:fitDock,cash:(c,el)=>cashFlow.play(c,el),effects,world,money:c=>money(c,true),next:()=>{busy=false;nextOffer();},death:()=>{endLife(run,'健康耗尽');save();showGameOver();}},platform});v9.ext=v12;r13=new R13({run:()=>run,meta:()=>meta,started:()=>started,modal:()=>modalType,busy:()=>busy,platform,save,toast,money,cash:(c,el)=>cashFlow.play(c,el),refresh:()=>{refreshStyle();renderHud();}});r9=new R9({run:()=>run,meta:()=>meta,started:()=>started,modal:()=>modalType,platform,save,adBusy:()=>lifeUI?.adBusy});{const _play=cashFlow.play.bind(cashFlow);cashFlow.play=(c,el)=>{const r=_play(c,el);try{v12.feedback(c,el);}catch{}return r;};};v7=new V7({confirm:confirmDialog,fit:fitDock,run:()=>run,started:()=>started,motion:()=>meta.motion,save,toast,music,modal:()=>modalType,busy:()=>busy,refresh:()=>{refreshStyle();renderHud();},renderDock,open:openModal,close:forceClose,cash:(c,el)=>cashFlow.play(c,el),world,effects});preferences();refreshStyle();renderHud();renderDock();save();platform.ready();platform.context(run);pause();$('loading')?.remove();try{audioGate(music,sound,{zh:meta.lang!=='en',enabled:meta.music||meta.sound});}catch(e){console.warn(e);}onboarding=new Onboarding({platform,run:()=>run,meta:()=>meta,world,save,legacyOpen:()=>lifeUI.openLegacy(true),legacyBuy:id=>lifeUI.purchasePerk(id),legacySkin:id=>lifeUI.chooseSkin(id),close:forceClose,unlock:()=>{music.unlock();effects.unlock();},block:value=>{started=!value;pause();musicMode();},enter:()=>{renderHud();renderDock();world.resize();pause();if(run.ended&&!lifeUI?.hasPendingDeath())showGameOver();}});runIntro({meta,save,motion:meta.motion,done:()=>{document.documentElement.lang=meta.lang==='zh'?'zh-CN':'en';world.setLanguage(meta.lang);if(!run.offer?.pendingStake)world.setOffer(run.offer);renderHud();renderDock();onboarding.show();}});requestAnimationFrame(pulseClock);setInterval(renderMusicStatus,900);
if(platform.storageMode==='session')setTimeout(()=>toast(L('Preview mode: progress lasts for this session.','预览模式：进度仅保存在本次会话。')),1700);
}
window.addEventListener('upshift-art',()=>{try{globalThis.__upshiftArtRefresh?.();}catch(e){console.warn(e);}});
