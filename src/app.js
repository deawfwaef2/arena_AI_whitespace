...
let lifeUI,onboarding,designUI,streetUI,prosperityUI,v7,v9,v12;let started=false;
$('app').innerHTML=shell;
...
function pulseClock(now){clock();lifeUI?.tick();streetUI?.tick(now);prosperityUI?.tick(now);v7?.tick(now);v12?.tick(now);v9?.tick(now);renderChallengeHud();if(run.activeChallenge&&now-lastSave>1000)save();if(started&&!busy&&!modalType&&!adPlaying){if(run.ended&&!lifeUI?.hasPendingDeath())showGameOver();else if(pendingChallenge.length)showChallengeResult(pendingChallenge.shift());}requestAnimationFrame(pulseClock);}
