...
const defaults={lang:'en',theme:'minimalist',layout:'journal',device:'desktop',scale:1,sound:true,music:true,ambience:true,volume:.28,musicMode:'auto',motion:!matchMedia('(prefers-reduced-motion: reduce)').matches,low:false,name:'',records:[],bots:[],runCount:1};
function normalizeMeta(input={}){const m={...defaults,...input};m.lang=m.lang==='zh'&&(m.introLang||m.langSet)?'zh':'en';m.name=String(m.name||'').slice(0,20);m.theme=['minimalist','imperial','cyber','swiss'].includes(m.theme)?m.theme:'minimalist';m.scale=Math.max(.75,Math.min(1.25,Number(m.scale)||1));m.records=Array.isArray(m.records)?m.records.filter(x=>x&&Number.isFinite(x.peak)).slice(0,25):[];m.bots=Array.isArray(m.bots)?m.bots.filter(x=>x&&x.simulated&&Number.isFinite(x.peak)).slice(0,100):[];m.volume=Math.max(0,Math.min(1,Number(m.volume)||0));m.musicMode=['auto','city','rush'].includes(m.musicMode)?m.musicMode:'auto';initLegacy(m);return m;}
...
function preferences(){$('app').dataset.largeText=String(!!meta.largeText);effects.sound=meta.sound;effects.motion=meta.motion;effects.low=meta.low;effects.muted=platform.muted||document.hidden||adPlaying;world.motion=meta.motion;world.quality(meta.low);music.enabled=meta.music;music.setVolume(meta.volume);music.setMuted(platform.muted||adPlaying);music.setHidden(document.hidden);musicMode();sound.setSfx(meta.sound);sound.setAmbience(meta.ambience!==false);sound.setMuted(platform.muted||adPlaying);document.body.dataset.motion=meta.motion?'on':'off';document.body.dataset.theme=meta.theme||'minimalist';document.documentElement.dataset.theme=meta.theme||'minimalist';document.documentElement.style.setProperty('--ui-scale',meta.scale||1);}
