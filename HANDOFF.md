...
## R15 progress log (2026-09-24, append-only)
- **Deliverable is now:** `index.html` + `art-pack.js` + `art-cities.js` + `music-pack/*.mp3` + `sfx-pack/*.js` (ZIP: `broke-to-billionaire-crazygames.zip`, rebuild with `python3 tools/make-crazygames-zip.py`). index.html alone still plays (silent, fallback art).
- 150 ✅(partial, measured) Load: JS emitted as UTF-8 (was \uXXXX escapes, −190 KB); portraits `ic-p-*` + mini-game `mg-*` art moved to art-pack; the 5 non-start city panoramas moved to `art-cities.js` (loaded 1.5 s after art-pack). index.html 3.03→2.55 MB (gzip 1.26→1.04 MB), art-pack 4.1→2.1 MB. qa/r15load.mjs (4 Mbps, CPU×4, no gzip): game ready 9.3→8.5 s. Remaining time is CPU (HTML/CSS parse, three.js init) — next levers: prune the 440 KB layered CSS, lazy-load i18n-en (222 KB) for zh players.
- 151 ✅ Main menu: BGM "Life of Riley" (Kevin MacLeod, CC BY 4.0, `assets/music/MENU-CREDITS.json`) plays on the menu (`musicMode()` → 'menu' while !started). Browsers block autoplay, so if `play()` is rejected a gold "Tap to start / 点击开始" veil (`#r15-gate`, src/sound.js `audioGate`) sits over the menu; the tap starts the BGM. Recorded click/hover/confirm/back SFX (Kenney CC0) on EVERY button via a document-level click listener (menu handlers used to return before the old synth tone).
- 152 ✅ Phone BGM: music is now streamed with `<audio>` from plain MP3 files (music-pack/*.mp3; old base64 JS packs deleted). No 60 MB PCM decode, starts after a few KB, and plays through the iPhone silent switch. All audio unlocks on pointerdown/touchend/click/keydown (capture). WebAudio (SFX/ambience) requests `navigator.audioSession.type='playback'`.
- 153 ✅ Phone landscape: virtual canvas 560 px tall (was 920) → ~0.7 scale on a 390 px phone; compact money/threshold cards (threshold icon row hidden), event panel below the HUD, story modal fits. Styles: `html[data-vscale="phone"]` in src/r14.css.
- 154 ✅ Desktop: same body-scale mechanism in 'desk' mode, zoom = min(1.3, H/700, W/1180) (off below 1.04). 1920×1080 → 1.3×.
...
### R13 item 142 — trailers re-rendered (2026-09-24)
- Root `promo-1920x1080.mp4` (11.5 MB) and `promo-1080x1920.mp4` (8.3 MB), both 13.1 s long, 30 fps, with vegas.mp3 music. Made from real gameplay frames captured with `qa/capture-r13.mjs` (outcomes rigged; one capture at a time, because 2 in parallel run the 2 GB sandbox out of memory) and composited by `tools/trailer-r13.py h|v` (env `FRAMES=` for the frames dir, `ONLY=` for chosen scenes).
- Scenes: tap work → camera follows the money to the top-left counter; Level up (chain unlock); invest slider → big profit; bigger bets in Vegas (+ chain unlock ×9); MAX all-in → bankrupt; end card "$100 → $1,000,000,000? PLAY FREE". Scenes are joined with zoom-crossfades, no hard cuts. Vertical version: the foreground crop slides left while following the cash so the whole counter stays in view; the end-card title art is fit to the frame width.
- Checked visually through contact sheets of frames extracted from both mp4s.
- [x] 160 (R16b) User: after -70% the SFX were TOO QUIET, ambience level is right → SFX restored to original loudness, ambience kept at the quieter level; added 3 sliders (music / sound effects / street ambience) in Settings AND the Music panel (meta.sfxVolume default 1, meta.ambVolume default .5; ambBus = .54*ambVol).
