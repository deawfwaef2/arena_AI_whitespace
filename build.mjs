...
function guardMedia(src){let out='',i=0;while(i<src.length){const m=src.indexOf('@media',i);if(m<0){out+=src.slice(i);break;}out+=src.slice(i,m);const b=src.indexOf('{',m);const cond=src.slice(m,b);let d=1,k=b+1;while(k<src.length&&d){if(src[k]==='{')d++;else if(src[k]==='}')d--;k++;}const body=src.slice(b+1,k-1);
 if(/width|height|orientation/.test(cond)){out+=cond+'{'+body.replace(/(^|})([^{}@]+)\{/g,(all,pre,sel)=>pre+sel.split(',').map(x=>{x=x.trim();if(/^(from|to|\d+%)$/.test(x))return x;if(x.startsWith(':root'))return 'html:not([data-vscale])'+x.slice(5);if(/^html(?![-\w])/.test(x))return 'html:not([data-vscale])'+x.slice(4);return 'html:not([data-vscale]) '+x;}).join(',')+'{')+'}';}else out+=cond+'{'+body+'}';i=k;}return out;}
